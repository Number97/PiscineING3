function loadItemsEnVente(filtrer) {
	...
	sql	= "select count(id) from panier where panier.client=" + session_id;
	...
	$.get("../PHP-GET.php", {"data" : sql}, (data) => {
		if (data.match("Error :") != null) { // Error. Reload and ask to connect again ?
			...
			sql="select * from item, panier where item.id = panier.item and panier.client = "+session_id;
			...
			$.get("../PHP-GET.php", {"data" : sql}, (data) => {
				if (data.match("Error :") != null) { // Error. Reload and ask to connect again ?
					...
					for(var indexCounter=0; indexCounter<nombreArticles;indexCounter++)
					{
						...
						panierTypeVentes[indexCounter] = ajust(arr[19+22*indexCounter]);
