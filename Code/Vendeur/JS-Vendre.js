function loadItemsEnVente(filtrer) {
	...
	sql="select count(id) from item where item.vendeur="+session_id+" and item.vendu=0";
	...
	$.get("../PHP-GET.php", {"data" : sql}, (data) => {
		if (data.match("Error :") != null) { // Error. Reload and ask to connect again ?
			...
			sql="select * from item where item.vendeur="+session_id+" and item.vendu=0";
...
function loadItemsVendu(filtrer) {
	...
	sql="select count(id) from item where item.vendeur="+session_id+" and item.vendu=1";
	...
	$.get("../PHP-GET.php", {"data" : sql}, (data) => {
		if (data.match("Error :") != null) { // Error. Reload and ask to connect again ?
			...
			sql="select * from item where item.vendeur="+session_id+" and item.vendu=1";
